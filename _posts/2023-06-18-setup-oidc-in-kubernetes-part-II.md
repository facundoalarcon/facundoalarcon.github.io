---
layout: post
title:  "Setup OIDC in Kubernetes clusters - Part II"
date:   2023-06-18 18:34:00 -0300
categories: kubernetes authentication authorization eks
---
In this post we will discuss how to setup OIDC in EKS clusters, based on the [previous post]({{ site.baseurl }}{% link _posts/2023-06-18-setup-oidc-in-kubernetes-part-I.md %}) . All this actions modify your kubeconfig file.
...
identityProviders:
 - name: auth0
   ...
   clientId: CLIENT_ID
   usernameClaim: http://localhost/ad_username
   groupsClaim: http://localhost/ad_groups
```
then execute
```bash
...
```js
resource "kubernetes_cluster_role_binding" "sec_role_binding" {
  metadata {
    name = "secrolebinding"
  }
  role_ref {
...
You can lear more about RBAC in this [link](https://kubernetes.io/docs/reference/access-authn-authz/rbac/).

The connection mechanism is the same as discussed in Part I.
